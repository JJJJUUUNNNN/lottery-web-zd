...
function setPrizes(pri) {
  prizes = pri;
  defaultType = prizes[0] ? prizes[0]["type"] : null;

  if (!defaultType) {
    console.error("Error: Unable to set defaultType. Check the 'type' property of the first prize object.");
  }

  lasetPrizeIndex = pri.length - 1;
  console.log("Prizes set successfully:", prizes);
}
...
function showPrizeList(currentPrizeIndex) {
  ...
  prizes.forEach((item,index) => {
    ...
    const __currentPrize = prizes[index];
    var totalCount = __currentPrize.count;
...
let selectedPriceLevel = 1; // 默认选中第一等奖
...
function setPrizeLevel(level) {
  if (level >= 1 && level <= 4) {
    ...
    console.error("Invalid price level. Please choose a level between 1 and 4.");
  }
}
